...
alias clock='tock --color 255 --military --center'
...
alias pdf='tdf'
alias math='_study_open Math'
alias japanese='_study_open Japanese'
alias physics='_study_open Physics'
alias kbd="hyprctl getoption input:kb_layout -j | grep -q '\"str\": \"us\"' && hyprctl eval 'input:kb_layout = jp' || hyprctl eval 'input:kb_layout = us"
alias fastfetch='fastfetch --config'
alias help='tldr'
alias cat='bat'
alias ansi='for i in {0..15}; do printf "\e[48;5;${i}m  %02d  \e[0m" "$i"; [ $(((i + 1) % 8)) -eq 0 ] && echo; done'
...
alias ogg='for f in *.*; do [[ "$f" == *.ogg ]] && continue; [[ -f "$f" ]] && ffmpeg -i "$f" -map_metadata 0 -c:a libvorbis -qscale:a 10 "${f%.*}.ogg" 2>/dev/null && echo "done: ${f%.*}.ogg"; done'
...
ansi256() {
     for i in {0..255}; do printf "\e[48;5;${i}m\e[38;5;$([ $i -gt 243 ] || [ $i -gt 123 -a $i -lt 232 ] && echo 0 || echo 15)m %3d \e[0m" "$i"; [ $(((i + 1) % 16)) -eq 0 ] && echo; done
}
...
function _study_open() {
  local subject_dir="$1"
  local origin_dir="$PWD"
  local study_base="/mnt/storage/Documents/Study"

  local target_dir=""
  if [[ -d "./${subject_dir}" ]]; then
    target_dir="./${subject_dir}"
  elif [[ -d "${study_base}/${subject_dir}" ]]; then
    target_dir="${study_base}/${subject_dir}"
  else
    echo "エラー: '$subject_dir' ディレクトリが見つかりません。"
    return 1
  fi
  ...
  local selected_file
  selected_file=$(find . -mindepth 1 -type f | sort | fzf --no-sort --prompt="${subject_dir}> ")
  ...
  if [[ -n "$selected_file" ]]; then
    ...
    case "$ext" in
      ...
      *)
        echo "未対応の拡張子です: $selected_file"
        ;;
    esac
  fi

  cd "$origin_dir" || return 1
}
